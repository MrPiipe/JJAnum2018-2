...
export class CroutPage {
  ...
  theoricHelp() {
    const text = `The direct factorization method of Crout pretends to decompose a matrix A into the
    product of the two matrices L (lower triangular matrix) and U (upper triangular
    matrix), so that LU = A. For that purpose, we begin with one lower triangular matrix
    where the elements different from zero are unknown, and with a upper triangular
    one where the elements different from zero are unknown but the elements of the
    diagonal are one (1), so that U[i][i]. Then, we use the matrix multiplication concept
    to find both matrices unknown elements. Once we have L and U, progressive and
    regressive substitution are applied to solve the system of equations.`
  ...
  private calculate() {
    const url = 'http://127.0.0.1:8000/rest_api/crout/';
    const params = {
      A: this.matrixA,
      b: this.matrixB
    };
